<?php
    ...
    if ($_GET['1'] == '1') {
        header('Location: http://setshly/php/Report/reportSameCategory.php?category='.'Футболки'.'');
    };

    if ($_GET['2'] == '2') {
        header('Location: http://setshly/php/Report/reportSameCategory.php?category='.'Джинсы'.'');
    };

    if ($_GET['3'] == '3') {
        header('Location: http://setshly/php/Report/reportSameCategory.php?category='.'Носки'.'');
    };

    if ($_GET['4'] == '4') {
        header('Location: http://setshly/php/Report/reportSameCategory.php?category='.'Худи'.'');
    };

    $url = urldecode(((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER    ['REQUEST_URI']);

    $cut_search = substr($url, 58);

    if ($cut_search == 'Футболки') {
        $searchCategory = 1;
    } elseif ($cut_search == 'Джинсы') {
        $searchCategory = 2;
    } elseif ($cut_search == 'Носки') {
        $searchCategory = 3;
    } elseif ($cut_search == 'Худи') {
        $searchCategory = 4;
    }
?>
   <!DOCTYPE html>
   ...
   <body id="body">
       ...
       <main id="main">
           ...
           <form method="get" class="form-buttons"><button class="form-button" name="1" value="1">Футболка</button><button class="form-button" name="2" value="2">Джинсы</button><button class="form-button" name="3" value="3">Носки</button><button class="form-button" name="4" value="4">Худи</button></form>
           <table class="html-table">
               ...
               <tbody>
                    <?php
                        ...
                        if (count($products) != 0) {
                            ...
                            for ($i = 0; $i <= count($products); $i++) {
                                if ($products[$i][1] == $searchCategory) {
                                    $productsSameCategory[$i] = $products[$i][0];
                                };
                            };

                            if ($productsSameCategory) {
                                $productsSameCategory = array_combine(range(1, count($productsSameCategory)), $productsSameCategory);
                            };
                    ...
                    if ($productsSameCategory) {
                        for ($i = 0; $i <= count($productsSameCategory); $i++) {
                            trHTML($productsSameCategory[$i]);
